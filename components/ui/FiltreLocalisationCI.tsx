...
// ============================================================================
// FiltreLocalisationCI — sélecteur de zone géographique
// avec carte réelle de la Côte d'Ivoire. S'adapte au pays sélectionné.
// ============================================================================
...
const COMMUNES_ABIDJAN = [
  "Cocody", "Yopougon", "Marcory", "Abobo", "Koumassi", 
  "Adjamé", "Port-Bouët", "Attécoubé", "Plateau", "Treichville",
];

const VILLES_INTERIEUR = [
  "Bouaké", "Yamoussoukro", "San-Pédro", "Korhogo", "Daloa", "Man", "Gagnoa",
];

const VILLES_AUTRES_PAYS: Record<string, string[]> = {
  SN: ["Dakar", "Thiès", "Saint-Louis", "Ziguinchor", "Touba", "Mbour", "Kaolack"],
  CM: ["Yaoundé", "Douala", "Garoua", "Bamenda", "Maroua", "Bafoussam"],
  ML: ["Bamako", "Sikasso", "Mopti", "Koutiala", "Kayes", "Ségou"],
  BF: ["Ouagadougou", "Bobo-Dioulasso", "Koudougou", "Banfora", "Ouahigouya"],
};

// Coordonnées SVG des grandes villes (CI uniquement)
...
export function FiltreLocalisationCI({ onSelectLocalisation }: FiltreLocalisationCIProps) {
  const { paysSelectionne } = useCountry();
  const labelToute = `Tout(e) ${paysSelectionne.nom}`;
  
  const [selection, setSelection] = useState(labelToute);
  ...
  // Mettre à jour la sélection si le pays change
  useEffect(() => {
    setSelection(labelToute);
  }, [paysSelectionne, labelToute]);

  const isCI = paysSelectionne.code === "CI";

  const listeActive = isCI
    ? (onglet === "abidjan" ? COMMUNES_ABIDJAN : VILLES_INTERIEUR)
    : (VILLES_AUTRES_PAYS[paysSelectionne.code] || []);
  ...
  return (
    <div className="mx-auto w-full max-w-5xl overflow-hidden rounded-xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/40 sm:p-8">
      <div className="grid gap-8 lg:grid-cols-12 lg:items-center">
        {/* ---------------------------------------------------------------- */}
        {/* Colonne gauche : recherche + sélection par badges                */}
        {/* ---------------------------------------------------------------- */}
        <div className={isCI ? "lg:col-span-7" : "lg:col-span-12"}>
          <div className="mb-5 flex items-center gap-2.5">
            <span className="flex size-9 items-center justify-center rounded-full bg-brand-500/10 text-brand-500 shadow-sm">
              <MapPin className="size-4.5" />
            </span>
            <h2 className="font-display text-xl font-semibold text-slate-900">
              Où cherchez-vous en {paysSelectionne.nom} ?
            </h2>
          </div>
          ...
          {/* Badges de zones — mode clair, bordures nettes */}
          <div className="flex max-h-44 flex-wrap gap-2 overflow-y-auto pr-1">
            <BadgeZone
              label={labelToute}
              actif={selection === labelToute}
              onClick={() => choisir(labelToute)}
            />
          ...
          <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4 text-sm">
            <span className="text-slate-500 font-medium">Zone active</span>
            <span className="rounded-md bg-brand-500/10 px-3 py-1.5 font-bold text-brand-500 shadow-sm">
              {selection}
            </span>
          </div>
        </div>
        ...
        {isCI && (
          <div className="lg:col-span-5 relative">
            <div className="absolute inset-0 bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-3xl -m-4 pointer-events-none" />
            <div className="relative mx-auto aspect-[400/460] w-full max-w-[280px] rounded-xl bg-white p-3 shadow-sm border border-slate-100">
              <svg
                viewBox={`0 0 ${VIEWBOX.w} ${VIEWBOX.h}`}
                className="size-full drop-shadow-sm"
              >
                {/* Fond de carte géopolitique léger */}
                <path
                  d={TRACE_PAYS}
                  className="fill-slate-100 stroke-slate-300 transition-colors duration-300"
                  strokeWidth="2.5"
                    ...
                    >
                      ...
                      <circle
                        ...
                        className={cn(
                          ...
                          actif
                            ? "fill-brand-500 stroke-brand-200"
                            : "fill-slate-400 hover:fill-brand-400 stroke-white"
                        )}
              ...
              {villeSurvolee && (
                <div
                  className="pointer-events-none absolute -translate-x-1/2 -translate-y-full rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs font-bold text-slate-800 shadow-xl"
                  style={{
                    left: `${
...
// Badge de zone cliquable — Mode clair
function BadgeZone({
  label,
  actif,
  onClick,
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-semibold shadow-xs transition-colors",
        actif
